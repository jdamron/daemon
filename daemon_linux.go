...
func newDaemon(name, user, description string, kind Kind, dependencies []string) (Daemon, error) {
	// newer subsystem must be checked first
	if _, err := os.Stat("/run/systemd/system"); err == nil {
		return &systemDRecord{name, user, description, kind, dependencies}, nil
	}
